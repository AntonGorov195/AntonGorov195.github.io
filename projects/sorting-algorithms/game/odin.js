...
(function () {
	...
	function odinSetupDefaultImports(wasmMemoryInterface, consoleElement, memory) {
		...
		return {
			...
			"webgl2": webglContext.getWebGL2Interface(), "local_storage": {
				_write_local_storage: (key_ptr, key_len, value_ptr, value_len) => {
					let key = wasmMemoryInterface.loadString(key_ptr, key_len);
					let value = wasmMemoryInterface.loadString(value_ptr, value_len);
					localStorage.setItem(key, value)
				},
				_read_local_storage_length: (key_ptr, key_len) => {
					let key = wasmMemoryInterface.loadString(key_ptr, key_len);
				...
				_read_local_storage: (key_ptr, key_len, addr) => {
					let key = wasmMemoryInterface.loadString(key_ptr, key_len);
					const value = localStorage.getItem(key)
					wasmMemoryInterface.storeString(addr, value)
				},
			},
		};
	};
